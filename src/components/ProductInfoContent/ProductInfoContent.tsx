...
import {
  ...
  MockupsGrid,
  ScrollableMain,
} from "./ProductInfoContent.styled";
...
export const ProductAboutContent = () => (
  <DefaultContent title="О ПРОДУКТЕ">
    ...
    <Description>
      Решение поддерживает полный цикл — от регистрации и аккредитации
      компании до создания предложения, переписки по заказу и сдачи отчёта с
      оценкой сервиса.
    </Description>
    <Description>
  ...
  <ScrollableMain>
    <DefaultContent title="ТЕХНИЧЕСКИЕ ХАРАКТЕРИСТИКИ">
      <Description>
        ...
        заказы, чаты, отчёты и профиль — реализована через React Router;
        визуальное оформление — на Styled Components, сборка и локальная
        разработка — через Vite.
      </Description>
      <Description>
        Серверная часть — REST API на Java и Spring Framework. Spring Boot
        обеспечивает конфигурацию сервиса, внедрение зависимостей и
        единообразную обработку HTTP-запросов; Spring Web — контроллеры и
        маршрутизацию эндпоинтов. Бизнес-логика (регистрация, аккредитация,
        предложения, заказы, отчёты) сосредоточена в сервисном слое; доступ к
        данным — через Spring Data и реляционную СУБД. Авторизация и сессии —
        на основе JWT или сессионных механизмов Spring Security.
      </Description>
      <Description>
        ...
        покупатель»: разные сценарии в одном приложении при общей модели
        пользователя и едином API. Для демонстрации и прототипирования
        допускается подмена бэкенда mock-сервером; целевой контур —
        развёртывание Spring-приложения и статики SPA за reverse proxy.
      </Description>
    </DefaultContent>
